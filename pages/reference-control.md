...
There are 121, grouped by what they are for rather than alphabetically.
...
**Nodes and the map** — `map.basemap` · `map.filter` · `map.layer` · `map.layers` · `map.zoom` · `node.energy` · `node.provisioning` · `node.radio` · `node.start` · `node.stop` · `node.truerf` · `nodes.keep` · `nodes.move` · `nodes.place` · `nodes.select` · `nodes.stats`

**Building a network** — `feed.pull` · `feed.stop` · `import.commit` · `import.describe` · `import.fetch` · `infer.apply` · `infer.run` · `project.new` · `project.open`

**Firmware and provisioning** — `console.cli` · `console.read` · `console.type` · `firmware.build` · `firmware.delete` · `firmware.download` · `firmware.import` · `firmware.installed` · `firmware.library` · `firmware.needed` · `firmware.set` · `firmware.start` · `firmware.state` · `firmware.wipe`

**Experiments and sweeps** — `bench.drop` · `bench.refresh` · `bench.serve` · `bench.stray` · `experiment.base` · `experiment.compare` · `experiment.define` · `experiment.export` · `experiment.results` · `experiment.seeds` · `experiment.senders` · `experiment.start` · `experiment.state` · `experiment.stop` · `experiment.vary` · `run.save` · `schedule.add` · `schedule.clear`
...
**Analysis** — `plan.routes` · `rf.environment` · `rf.mode` · `rf.realism` · `rf.toggle` · `terrain.cache` · `terrain.prefetch`

**Everything else** — `assert.add` · `assert.check` · `board.key` · `board.press` · `board.screen` · `board.touch` · `gpu.set` · `gpu.state` · `job.cancel` · `layout.reset` · `links.recompute` · `packet.close` · `packet.open` · `radio.preset` · `study.margin` · `tool.set`
